...
它会自动等待一个 awaitable 的结果，而不是像普通 for 那样只迭代已经就绪的对象。
所以可以对接「流式、逐步产生结果」的场景（网络流、数据库流、异步生成器）。
"""
...
async def download_single_image(
    ...
    # 只接收 response 的 header 部分，body 部分不一定到达了
    ...
    async with aiofiles.open(download_path, "wb") as f:
        # 接收 response 的 body 部分
        # async for: 每次迭代会 await 异步迭代器的 __anext__()，等数据准备好再返回给 chunk
        async for chunk in response.aiter_bytes(chunk_size=8192):
            await f.write(chunk)  # 再次异步写文件 (共计三个异步操作)
